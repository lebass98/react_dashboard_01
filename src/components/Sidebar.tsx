    ...
    >
      ...
      <div className="flex-1 overflow-y-auto py-6 px-4 space-y-1 custom-scrollbar shadow-[0_25px_50px_-12px_#00000040] bg-slate-50/30">
        ...
        <div className="group flex items-center justify-between px-3 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white shadow-md shadow-indigo-200 cursor-pointer">
          ...
          <span className="text-[10px] font-bold bg-white/20 px-2 py-0.5 rounded text-white tracking-wide">NEW</span>
        </div>
  ...
  return (
    <div>
      <div 
        ...
        className={`group flex items-center justify-between px-3 py-2.5 rounded-xl text-slate-500 hover:text-indigo-600 hover:bg-indigo-50/50 cursor-pointer transition-all ${isOpen ? 'bg-indigo-50/30' : ''}`}
      >
        <div className="flex items-center gap-3">
        ...
        >
          <div className="flex flex-col gap-1 pl-11 pr-3 py-1">
            {subItems.map((item, idx) => (
              <div 
                key={idx} 
                className="text-sm font-medium text-slate-500 hover:text-indigo-600 py-1.5 cursor-pointer transition-colors"
              >
                {item}
              </div>
            ))}
