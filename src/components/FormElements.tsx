...
const FormElements: React.FC = () => {
  ...
  const [radioValue, setRadioValue] = React.useState('secondary');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-20">
          ...
          <div className="bg-white rounded-xl border border-slate-100 p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} placeholder="Enter your password" className="w-full px-4 py-3 rounded-lg border border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none transition-all" />
                  <Eye 
          ...
          <div className="bg-white rounded-xl border border-slate-100 p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <div 
                    className="w-full px-4 py-3 rounded-lg border border-slate-200 flex flex-wrap gap-2 items-center bg-white cursor-pointer min-h-[50px] transition-all relative z-10"
                  ...
                  >
                    ...
                    {selectedOptions.map(option => (
                      <span key={option} className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-xs font-semibold flex items-center gap-1">
                        {option} 
                        <X 
                  ...
                  {/* Dropdown Menu */}
                  {multiSelectOpen && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-20 max-h-48 overflow-y-auto custom-scrollbar">
                      {availableOptions.map(option => (
                        <div 
                          key={option}
                          onClick={() => toggleOption(option)}
                          className={`px-4 py-2 cursor-pointer hover:bg-slate-50 flex items-center transition-colors ${selectedOptions.includes(option) ? 'bg-indigo-50/50 text-indigo-600 font-medium' : 'text-slate-600'}`}
                        >
                          <div className={`w-4 h-4 rounded border mr-3 flex items-center justify-center transition-colors ${selectedOptions.includes(option) ? 'border-indigo-500 bg-indigo-500' : 'border-slate-300'}`}>
                            {selectedOptions.includes(option) && <div className="w-1.5 h-1.5 rounded-sm bg-white" />}
                          </div>
                          {option}
                        </div>
                      ))}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-100 p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex">
                  <input type="text" value="www.tailadmin.com" readOnly className="flex-1 px-4 py-3 rounded-l-lg border border-slate-200 bg-white outline-none text-slate-600" />
                  <button 
                    onClick={handleCopy}
                    className="px-4 py-3 rounded-r-lg border border-l-0 border-slate-200 bg-slate-50 text-indigo-500 font-semibold flex items-center gap-2 hover:bg-slate-100 transition-colors"
                  >
          ...
          <div className="bg-white rounded-xl border border-slate-100 p-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <label htmlFor="file-upload" className="flex items-center w-full px-4 py-3 rounded-lg border border-slate-200 cursor-pointer hover:border-indigo-500 transition-all overflow-hidden whitespace-nowrap">
                  <span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-md text-sm font-semibold mr-4 shrink-0">파일 선택</span>
          ...
          <div className="bg-white rounded-xl border border-slate-100 p-6 space-y-6">
            ...
            <div className="flex flex-wrap gap-8 items-center pt-2">
              <label className="flex items-center gap-2 cursor-pointer group">
                ...
                <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${radioValue === 'default' ? 'border-indigo-500 bg-indigo-500' : 'border-slate-300 group-hover:border-indigo-500'}`}>
                   {radioValue === 'default' && <div className="w-1.5 h-1.5 rounded-full bg-white"></div>}
                </div>
                <span className={`text-sm font-medium ${radioValue === 'default' ? 'text-slate-800' : 'text-slate-600'}`}>Default</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer group">
                ...
                <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${radioValue === 'secondary' ? 'border-indigo-500 bg-indigo-500' : 'border-slate-300 group-hover:border-indigo-500'}`}>
                  {radioValue === 'secondary' && <div className="w-1.5 h-1.5 rounded-full bg-white"></div>}
                </div>
                <span className={`text-sm font-medium ${radioValue === 'secondary' ? 'text-slate-800' : 'text-slate-600'}`}>Secondary</span>
              </label>
              <label className="flex items-center gap-2 cursor-not-allowed opacity-50">
            ...
            <div className="space-y-4 pt-2">
              <div className="flex gap-8 items-center">
                <label className="flex items-center gap-3 cursor-pointer group" onClick={() => setToggle1(!toggle1)}>
                  <div className={`relative w-11 h-6 rounded-full transition-colors ${toggle1 ? 'bg-indigo-500' : 'bg-slate-200 group-hover:bg-slate-300'}`}>
                  ...
                  <span className={`text-sm font-medium ${toggle1 ? 'text-indigo-600' : 'text-slate-600'}`}>Default</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer group" onClick={() => setToggle2(!toggle2)}>
                  <div className={`relative w-11 h-6 rounded-full transition-colors ${toggle2 ? 'bg-indigo-500' : 'bg-slate-200 group-hover:bg-slate-300'}`}>
                  ...
                  <span className={`text-sm font-medium ${toggle2 ? 'text-indigo-600' : 'text-slate-600'}`}>Checked</span>
                </label>
              ...
              <div className="flex gap-8 items-center">
                <label className="flex items-center gap-3 cursor-pointer group" onClick={() => setToggle3(!toggle3)}>
                  <div className={`relative w-12 h-6 border-2 rounded-full transition-colors flex items-center px-1 ${toggle3 ? 'border-slate-800' : 'border-slate-200 group-hover:border-slate-300'}`}>
                    <div className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${toggle3 ? 'bg-slate-800 translate-x-6' : 'bg-slate-200 translate-x-0'}`}></div>
                  </div>
                  <span className={`text-sm font-medium ${toggle3 ? 'text-slate-800 font-bold' : 'text-slate-600'}`}>Default</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer group" onClick={() => setToggle4(!toggle4)}>
                   <div className={`relative w-12 h-6 border-2 rounded-full transition-colors flex items-center px-1 ${toggle4 ? 'border-slate-800' : 'border-slate-200 group-hover:border-slate-300'}`}>
                    <div className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${toggle4 ? 'bg-slate-800 translate-x-6' : 'bg-slate-200 translate-x-0'}`}></div>
                  </div>
                  <span className={`text-sm font-medium ${toggle4 ? 'text-slate-800 font-bold' : 'text-slate-600'}`}>Checked</span>
                </label>
                <label className="flex items-center gap-3 cursor-not-allowed opacity-50">
          ...
          <div className="bg-white rounded-xl border border-slate-100 p-6 space-y-6">
            ...
            <div 
              className={`border-2 border-dashed rounded-xl p-10 flex flex-col items-center justify-center text-center space-y-4 cursor-pointer transition-all ${dragActive ? 'border-indigo-500 bg-indigo-50/50 scale-[1.02]' : 'border-indigo-100 bg-indigo-50/5 hover:border-indigo-300'}`}
            ...
            >
              <input 
                ...
                multiple
              />
              <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${dragActive ? 'bg-indigo-100' : 'bg-slate-100'}`}>
